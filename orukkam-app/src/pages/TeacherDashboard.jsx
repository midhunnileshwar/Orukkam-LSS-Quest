            ...
            <div className="container mx-auto p-6 grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <nav className="space-y-2">
                    <button
                        ...
                        className={`w-full flex items-center space-x-3 p-3 rounded-xl transition-all ${activeTab === 'magic-sheet' ? 'bg-indigo-600 text-white shadow-lg' : 'hover:bg-slate-800 text-slate-400'}`}
                    >
                    ...
                    <button
                        ...
                        className={`w-full flex items-center space-x-3 p-3 rounded-xl transition-all ${activeTab === 'smart-paste' ? 'bg-indigo-600 text-white shadow-lg' : 'hover:bg-slate-800 text-slate-400'}`}
                    >
                        <FileText size={20} />
                ...
                <main className="lg:col-span-3 space-y-6">
                    ...
                    {activeTab === 'magic-sheet' && (
                        ...
                        >
                            ...
                            <p className="text-slate-400 mb-6">Paste a shared Google Sheet URL to bulk import questions.</p>
                            ...
                            <div className="bg-slate-900/50 rounded-xl p-4 border border-dashed border-slate-700 text-center">
                                <p className="text-sm text-slate-500">Supported columns: Question, Option A, Option B, Correct, Explanation</p>
                            </div>
                        </motion.div>
                    )}
                        ...
                        >
                            ...
                            {parsedQuestions.length > 0 && (
                                <div className="space-y-4">
                                    <h3 className="font-bold text-slate-300 uppercase tracking-wider text-sm">Preview ({parsedQuestions.length})</h3>
                                    {parsedQuestions.map((q) => (
                                        <div key={q.id} className="bg-slate-700/50 p-4 rounded-xl border border-slate-600 flex justify-between items-start">
                                            <div>
                                                <p className="font-bold text-white mb-1">{q.text}</p>
                                                <span className="text-xs bg-green-900 text-green-300 px-2 py-1 rounded">Ans: {q.correct}</span>
                                            </div>
