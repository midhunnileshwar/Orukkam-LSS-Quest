...
export default function HomeMap() {
    ...
    const lands = [
        { id: 'maths', name: 'സംഖ്യാലോകം', color: 'from-amber-400 to-orange-500', icon: '🧮', x: 20, y: 30, status: getLandProgress('maths') },
        { id: 'malayalam', name: 'തേന്മൊഴി', color: 'from-lime-400 to-green-600', icon: '📝', x: 80, y: 30, status: getLandProgress('malayalam') },
        { id: 'english', name: 'English Tent', color: 'from-indigo-400 to-blue-600', icon: '⛺', x: 50, y: 50, status: getLandProgress('english') },
        { id: 'evs', name: 'പൂമ്പാറ്റ', color: 'from-emerald-400 to-teal-600', icon: '🦋', x: 25, y: 70, status: getLandProgress('evs') },
        { id: 'gk', name: 'ചോദ്യ കുടുക്ക', color: 'from-rose-400 to-red-600', icon: '💡', x: 75, y: 70, status: getLandProgress('gk') },
    ];
            ...
            <div className="relative w-full h-full min-h-screen flex flex-col">
                ...
                <div className="flex-1 relative w-full h-full overflow-hidden">
                    ...
                    <svg className="absolute inset-0 w-full h-full pointer-events-none z-0 opacity-40">
                        ...
                        <path d="M 50% 85% Q 20% 60% 20% 30%" stroke="white" strokeWidth="8" strokeDasharray="15,15" fill="none" />
                        <path d="M 50% 85% Q 80% 60% 80% 30%" stroke="white" strokeWidth="8" strokeDasharray="15,15" fill="none" />
                        <path d="M 50% 85% L 50% 50%" stroke="white" strokeWidth="8" strokeDasharray="15,15" fill="none" />
                        <path d="M 50% 85% Q 30% 80% 25% 70%" stroke="white" strokeWidth="8" strokeDasharray="15,15" fill="none" />
                        <path d="M 50% 85% Q 70% 80% 75% 70%" stroke="white" strokeWidth="8" strokeDasharray="15,15" fill="none" />
                    </svg>
                        ...
                        >
                            ...
                            `}>
                                ...
                                <div className="absolute -bottom-3 bg-white px-2 py-0.5 rounded-full border-2 border-slate-100 shadow-sm flex items-center space-x-1">
                                    ...
                                    <span className="text-[10px] font-bold text-slate-500">{Math.round(land.status)}%</span>
                                </div>
