...
export default function GearButton({ option, text, isSelected, result, onClick, disabled }) {
    ...
    const baseStyle = "relative w-36 h-36 flex flex-col items-center justify-center p-2 rounded-full transition-colors";
    let colorStyle = "bg-white border-8 border-slate-200 text-slate-600 shadow-cartoon hover:scale-105 active:scale-95";

    if (isSelected && result === null) colorStyle = "bg-blue-100 border-8 border-candy-blue text-candy-blue shadow-none";
    if (isSelected && result === 'correct') colorStyle = "bg-green-100 border-8 border-candy-green text-candy-green shadow-[0_0_30px_rgba(34,197,94,0.6)] scale-110 z-10";
    if (isSelected && result === 'wrong') colorStyle = "bg-red-100 border-8 border-candy-red text-candy-red shadow-none";
